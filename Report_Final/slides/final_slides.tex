...
\date{Tháng 5, 2026}
\titlegraphic{\includegraphics[height=1.2cm]{hcmut.png}}

% ============================================================
\begin{document}

\input{sections/1_opening}         % Title, Contribution, TOC
\input{sections/2_introduction}    % Bối cảnh, Bài toán, Mục tiêu, Dataset, Schema
\input{sections/3_theory}          % ES, BM25, Embedding/HNSW, RRF
\input{sections/4_architecture}    % System Architecture, Data Flow, Search Flow
\input{sections/5_embedding}       % Model Selection, GPU Pipeline, Checkpoint
\input{sections/6_search}          % BM25 Search, kNN Search, Hybrid RRF
\input{sections/7_evaluation}      % Methodology, Metrics, Results, Analysis
\input{sections/8_benchmarking}    % Latency, Scaling, Storage
\input{sections/9_conclusion}      % Summary, Limitations, Demo
\input{sections/10_closing}        % References, Q&A
\input{sections/11_backup}         % Backup slides
