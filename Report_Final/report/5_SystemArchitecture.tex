...
\node[storage, right=of estext] (esvec) {arxiv\_vectors\\{\scriptsize kNN Index 10.4GB}};
...
\node[process, below right=0.4cm and 1.5cm of api] (knnq) {kNN Search\\{\scriptsize cosine, k=10}};
...
\end{tikzpicture}
\caption{Luồng tìm kiếm Hybrid Search: BM25 và kNN chạy song song, gộp kết quả bằng RRF}
...
\begin{enumerate}
    ...
    \item \textbf{Luồng kNN:} Mô hình \texttt{all-MiniLM-L6-v2} chuyển câu truy vấn thành vector 384 chiều. Vector này được gửi đến chỉ mục \texttt{arxiv\_vectors} để tìm $k$ láng giềng gần nhất trên đồ thị HNSW.
...
\begin{itemize}
    ...
    \item \textbf{Index \texttt{arxiv\_vectors}:} Chứa toàn bộ metadata và vector nhúng 384 chiều (\texttt{embedding}), phục vụ tìm kiếm kNN. Kích thước 10.4GB.
\end{itemize}
