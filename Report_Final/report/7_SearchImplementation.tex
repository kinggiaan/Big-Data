...
\section{Truy vấn Tìm kiếm Vector tương đồng (kNN Search)}
...
\begin{itemize}
    ...
    \item \textbf{Định nghĩa chỉ mục kNN:} Trường \texttt{embedding} trong Elasticsearch được thiết lập sử dụng giải thuật HNSW (Hierarchical Navigable Small World) để tạo cấu trúc đồ thị tìm kiếm láng giềng gần nhất với hàm đo độ tương đồng Cosine (\texttt{similarity: "cosine"}).
...
Cú pháp DSL truy vấn kNN trong hệ thống được triển khai như sau:
\begin{lstlisting}[caption={Elasticsearch kNN Query DSL},label={lst:knn-dsl}]
{
  "knn": {
    "field": "embedding",
...
Để khắc phục nhược điểm của từng phương pháp tìm kiếm độc lập (BM25 bỏ sót các từ đồng nghĩa; kNN vector đôi khi bỏ sót các từ khóa chuyên ngành viết tắt chính xác), hệ thống triển khai cơ chế tìm kiếm kết hợp \textbf{Hybrid Search} sử dụng giải thuật gộp thứ hạng \textbf{Reciprocal Rank Fusion (RRF)}.
...
Trong đó:
\begin{itemize}
    \item $M$ là tập hợp các phương pháp tìm kiếm ($M = \{\text{BM25}, \text{kNN}\}$).
...
\begin{itemize}
    ...
    \item Truy vấn kNN được gửi đến chỉ mục \texttt{arxiv\_vectors} --- chứa cả metadata và vector nhúng 384 chiều.
    \item Sau khi gộp thứ hạng RRF, các tài liệu chỉ xuất hiện trong kết quả kNN sẽ được bổ sung metadata từ \texttt{arxiv\_text} thông qua lệnh \texttt{es.mget()} (Multi-Get API).
\end{itemize}
